...
export default function EditCoursePage() {
  ...
  return (
    <div className="flex flex-1 flex-col gap-6">
      ...
      {error ? (
        <div className="lg:ml-64 rounded-3xl bg-card p-4 text-sm text-brand-orange">
          {error}
        </div>
      ) : isLoading ? (
        <div className="lg:ml-64 h-64 animate-pulse rounded-3xl bg-card" />
      ) : (
        <>
          {deleteError ? (
            <div className="lg:ml-64 rounded-3xl bg-brand-orange/10 p-4 text-sm text-brand-orange">
              {deleteError}
            </div>
          ) : null}
          {course ? (
            <div className="lg:ml-64 flex justify-end">
              <button
                type="button"
