...
if [ -z "$USER_ID" ]; then
    ...
    echo "Использование: ./check-progress-final.sh <USER_ID>"
    exit 1
fi
...
# Выполняем скрипт в рабочей директории backend
...
async function checkProgress(userId) {
  console.log('🔍 Проверка прогресса для пользователя: ' + userId + '\\n');
